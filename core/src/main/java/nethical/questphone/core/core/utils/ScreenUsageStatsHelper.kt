...
class ScreenUsageStatsHelper(private val context: Context) {

    private val usageStatsManager =
        context.getSystemService(Context.USAGE_STATS_SERVICE) as UsageStatsManager

    private val guardian = UnmatchedCloseEventGuardian()
    fun getForegroundStatsByTimestamps(start: Long, end: Long): List<ScreentimeStats> {
    ...
    fun getForegroundStatsByRelativeDay(offset: Int): List<ScreentimeStats> {
    ...
    fun getForegroundStatsByDay(queryDate: LocalDate): List<ScreentimeStats> {
    ...
    fun getStatsForLast7Days(): List<ScreentimeStats> {
        val stats = mutableListOf<ScreentimeStats>()
        for (offset in 0..6) { // 0 = today, 6 = 6 days ago
            stats += getForegroundStatsByRelativeDay(offset)
        }
        return stats
    }
    ...
    private fun aggregateForegroundStats(foregroundStats: List<ComponentForegroundStat>): List<ScreentimeStats> {
        val usageScreentimeStats = mutableListOf<ScreentimeStats>()
        if (foregroundStats.isEmpty()) return usageScreentimeStats
        ...
        for ((packageName, totalTime) in applicationTotalForegroundTime) {
            ...
            usageScreentimeStats.add(ScreentimeStats(packageName, totalTime, startTimes))
        }

        // Sort by total time in descending order
        return usageScreentimeStats.sortedByDescending { it.totalTime }
    }
